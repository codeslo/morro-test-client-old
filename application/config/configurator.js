function config(testName) {
  ...
  jasmine.onComplete(function(passed) {
    ...
    axios
      .post("https://morroapp.com/api/test-results", {
      ...
      .then(response => {
        console.log(
          chalk.green(
            "Solution submitted. " +
              (response.data ? chalk.cyan(response.data) : "")
          )
        );
        if (passed) {
      ...
      .catch(err => {
        console.log(
          chalk.red("Submission failed. ") +
            "Error code: " +
            chalk.yellow(err.response.status) +
            " " +
            chalk.cyan(err.response.data)
        );
        process.exit();
      });
  });
  ...
  if (path.dirname(requester).includes("challenges")) {
    return console.log(
      chalk.red(
        "You're in the challenges directory. Please copy-paste your code into my_solutions. Code on!"
      )
    );
  }
